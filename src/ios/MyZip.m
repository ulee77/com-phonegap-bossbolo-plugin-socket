...
{
    NSData *pUncompressedData = [dataString dataUsingEncoding:NSUTF8StringEncoding];
...
//    Byte *dataByte = (Byte*)[compressedData bytes];
	return compressedData;
}

+(NSString *)uncompressZippedData:(NSArray *)dataArray  {
    ...
    NSData *compressedData = [[NSData alloc] initWithBytes:bytes length:dataCount];
    
    free(bytes);
    
//    NSData *compressedData = [Encryption dataWithArray:data];
	
	if ([compressedData length] == 0) return [Encryption stringWithData:compressedData];
    ...
    if (done) {  
        ...
        return [Encryption stringWithData:[NSData dataWithData: decompressed]];
    } else {  
        return nil;  
    }  
}
